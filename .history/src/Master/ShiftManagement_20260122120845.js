  ...
  useEffect(() => {
    const fetchShiftOptions = async () => {
      try {
        ...
        const activeShifts = res.data.filter(s => s.status === "Active");
        ...
        // Create DD combinations (e.g., MG, GA, AE...)
        const ddCombinations = [];
        for (let i = 0; i < options.length; i++) {
          for (let j = 0; j < options.length; j++) {
            if (i !== j) {
              const code1 = options[i].code;
              const code2 = options[j].code;
              ddCombinations.push({
                code: `${code1}${code2}`.toUpperCase(),
                name: `DD: ${options[i].name} + ${options[j].name}`
              });
            }
          }
        }
        ...
        setShiftOptions([
          ...options,
          { code: "OFF", name: "OFF" },
          ...ddCombinations // Add all generated DD pairs here
        ]);
