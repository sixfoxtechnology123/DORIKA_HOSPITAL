...
const calculateDuration = (actualIn, actualOut, shiftStart, shiftEnd) => {
  ...
  // --- Rule for START TIME ---
  // If within 15m grace, use Shift Start. Otherwise, use the Late Actual Time.
  if (actInMin >= shStartMin - 15 && actInMin <= shStartMin + 15) {
    finalStart = shStartMin; 
  } else {
    finalStart = actInMin;
  }

  // --- Rule for END TIME ---
  // If within 30m grace after shift, use Shift End. Otherwise, use Actual Out.
  if (adjustedOutMin >= shEndMin && adjustedOutMin <= shEndMin + 30) {
    finalEnd = shEndMin;
  } else {
    finalEnd = adjustedOutMin;
  }
...
module.exports = { markDailyAttendance, getMyAttendance };const Attendance = require("../models/Attendance");
const Leave = require("../models/LeaveApplication");
const ShiftMaster = require("../models/Shift");
const ShiftManagement = require("../models/ShiftManagement");
