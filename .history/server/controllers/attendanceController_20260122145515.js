...
const markDailyAttendance = async (req, res) => {
  try {
   ...
   // --- STATUS & LATE ENTRY CALCULATION ---
    ...
    let lateEntry = false;
    const graceThreshold = startMin + 15; // 15 mins after shift starts

    if (currentMin > graceThreshold) {
      lateEntry = true;
      // todayStatus = "Present"; // Keep as present, or change to "Late" based on policy
    }

    // This is where you store the shift timings into the record
    attendance.records.push({
      date: todayStr,
      status: todayStatus,
      ...
      // SAVING THE TIMES FROM SHIFT MASTER HERE
