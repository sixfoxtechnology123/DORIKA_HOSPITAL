...
const calculateDuration = (actualIn, actualOut, shiftStart, shiftEnd) => {
  ...
  // --- Rule for IN ---
  let finalStart = (actInMin >= shStartMin - 15 && actInMin <= shStartMin + 15) ? shStartMin : actInMin;

  // --- Rule for OUT ---
  let finalEnd = (adjustedOutMin >= shEndMin && adjustedOutMin <= shEndMin + 30) ? shEndMin : adjustedOutMin;
 ...
 // 2. CHECK FOR CHECK-OUT
    ...
    if (todayIndex !== -1) {
      ...
      if (record.checkInTime && (!record.checkOutTime || record.checkOutTime === "" || record.checkOutTime === "--")) {
        ...
        // Pass stored shiftStartTime and shiftEndTime
