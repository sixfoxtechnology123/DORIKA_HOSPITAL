...
const publicApiRoutes = new Set([
  "POST:/admin/login",
  "POST:/adminManagement/login",
  "POST:/employee-ids/login",
]);

app.use("/api", (req, res, next) => {
  if (req.method === "OPTIONS") return next();
  const routeKey = `${req.method}:${req.path}`;
  if (publicApiRoutes.has(routeKey)) return next();
...
const createDefaultAdmin = async () => {
  try {
    const adminExists = await Admin.findOne({ userId: "dorika" });
    ...
    if (!adminExists) {
      ...
      console.log("Default admin created → userId: dorika | password: dorika123");
    } else {
