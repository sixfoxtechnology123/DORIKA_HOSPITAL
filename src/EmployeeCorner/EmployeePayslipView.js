...
const EmployeePayslipView = () => {
  ...
  const TwoColRow = ({ label1, value1 }) => (
    <div className="flex mb-1 text-xs">
      <div className="min-w-[140px] font-medium">{label1}</div>
  ...
  return (
    <>
      <div className="flex min-h-screen flex-col md:flex-row bg-gray-50">
        ...
        <div className="flex-1 overflow-y-auto p-3">
          <div className="bg-white shadow-md rounded-md p-3">
            <div className="bg-blue-50 border border-gray-300 rounded-lg p-2 mb-4 flex justify-between items-center">
              <h2 className="text-lg font-bold text-blue-800">My Pay Slips</h2>
              <div className="flex gap-2">
                <input
                  type="month"
                  className="border border-gray-300 py-1 px-2 rounded text-sm"
                  value={selectedMonth}
              ...
              <tbody className="text-center">
                {filteredPayslips.map((ps, i) => (
                  <tr key={ps._id} className="hover:bg-gray-50">
                    ...
                    <td className="border border-gray-300 px-2 py-2 font-bold text-green-700">
                      ₹{ps.netSalary}
                    </td>
                    <td className="border border-gray-300 py-2">
                      <div className="flex justify-center gap-4">
                        <button onClick={() => setViewPayslip(ps)} className="text-blue-600">
                          <FaEye />
                        </button>
                        <button onClick={() => handleDownloadPDF(ps)} className="text-green-600">
                          <FaDownload />
                        </button>
                      </div>
                    </td>
      ...
      {viewPayslip && (
        <div className="fixed inset-0 bg-black/70 z-50 flex justify-center items-start overflow-y-auto pt-6 pb-10">
          <div className="bg-white w-[210mm] shadow-2xl relative">
            <button
              onClick={() => setViewPayslip(null)}
              className="absolute -top-2 -right-12 text-white"
            >
              <FaTimes size={32} />
            </button>
            ...
            <div
              id="print-section"
              className="border border-gray-800 m-10 px-4 py-2"
              style={{ fontFamily: "Arial, sans-serif" }}
            >
              <div className="flex justify-between items-center border-b border-gray-300 pb-3 mb-3">
                <img src={EyeLogo} className="w-20 h-20 rounded-full" />
                <div className="text-center">
                  <h1 className="text-2xl font-bold">DORIKA HOSPITAL</h1>
                  <p className="text-xs">NH 715, Mazgaon, Tezpur-784154</p>
                </div>
                <img src={EyeLogo} className="w-20 h-20 rounded-full  " />
              </div>
            ...
            <div className="border border-gray-800 px-4 py-2 mb-4 text-sm">
              <div className="mb-1 grid grid-cols-3 gap-4 items-start">
                ...
                <div className="col-span-2 space-y-1">
                  <TwoColRow
                    ...
                    value1={viewPayslip.employeeName}
                  />
                  <TwoColRow
                    label1="Employee ID"
                    value1={viewPayslip.employeeId}
                  />
                  <TwoColRow
                    label1="Designation"
                    value1={viewPayslip.designationName}
                  />
                  <TwoColRow
                    label1="Date of Joining"
                    value1={viewPayslip.doj}
                  />
                  <TwoColRow
                    label1="Pay Month"
                    value1={`${viewPayslip.month} ${viewPayslip.year}`}
                  />
                </div>

                {/* Right Section (1/3) — Two values per row */}
                <div className="col-span-1 border border-gray-300 rounded px-4 py-2 bg-green-50 text-left">
                  
                  <p className="font-semibold">Total Payable: ₹{viewPayslip.netSalary}</p>

                  {/* <div className="flex justify-between">
                    <span className="text-sm">Working days: <b>{viewPayslip.totalWorkingDays}</b></span>
                    <span className="text-sm">Lop: <b>{viewPayslip.lopDays || 0}</b></span>
                  </div> */}
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="border border-gray-800">
                  <h3 className="bg-gray-100 font-bold text-xs p-2 border-b border-gray-300">
                    Earnings
                  </h3>
                    ...
                    <tbody>
                      ...
                      <tr className="bg-green-50 font-bold">
                        <td className="border border-gray-300 p-2">
                          OT ({viewPayslip.otHours || 0} hrs)
                        </td>
                        <td className="border border-gray-300 p-2 text-right">
                          ₹{viewPayslip.otAmount || 0}
                        </td>
                      </tr>
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      {(viewPayslip.deductions || []).length ? (
                        ...
                        <tr>
                          <td
                            colSpan="2"
                            className="border border-gray-300 p-6 text-center text-gray-400"
                          >
                            No Deductions
                          </td>
                        </tr>
