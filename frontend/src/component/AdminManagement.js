...
export default function AdminManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [userRes, empRes] = await Promise.all([
        axios.get("/api/adminManagement/users", { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const saveUser = async () => {
    ...
    try {
      if (isEditing) {
        await axios.put(`/api/adminManagement/users/${editingUserId}`, newUser, { headers: { Authorization: `Bearer ${token}` } });
        toast.success("User updated!");
      } else {
        await axios.post("/api/adminManagement/users", newUser, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const deleteUser = async (id) => {
    ...
    try {
      await axios.delete(`/api/adminManagement/users/${id}`, { headers: { Authorization: `Bearer ${token}` } });
