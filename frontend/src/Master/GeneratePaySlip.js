  ...
  const checkExistingPayslip = async () => {
    ...
    try {
      const res = await axios.get(
        `/api/payslips/employee?employeeUserId=${selectedEmployee.employeeUserId}&month=${month}&year=${year}`
      );
  ...
  const loadMasterData = async () => {
    ...
    try {
      ...
      const [salaryRes, attendanceRes, otRateRes] = await Promise.all([
        axios.get(`/api/payslips/employee/${selectedEmployee.employeeID}?month=${month}&year=${year}`),
        axios.get(`/api/attendance/history?month=${monthMap[month]}&year=${year}`),
        axios.get(`/api/ot/ot-rate/rule?employeeUserId=${selectedEmployee.employeeUserId}`)
      ]);
      ...
      toast.error("Error loading master data:", err);
    }
  };
...
//     try {
//       const res = await axios.get(
//         `/api/payslips/employee/${selectedEmployee.employeeID}?month=${month}&year=${year}`
//       );
...
//       }
//     } catch (err) {
//       toast.error("Error fetching payslip:", err);
//     }
//   };
...
const handleSave = async () => {
  ...
  try {
    if (editingData && editingData._id) {
      ...
      await axios.put(`/api/payslips/${editingData._id}`, payload);
  ...
  if (!el) {
    toast.error("print-section not found");
    return;
  }
