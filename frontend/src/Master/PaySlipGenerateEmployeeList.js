...
const [hasPayslipForMonth, setHasPayslipForMonth] = useState(false);
...
const handlePrintAllOnePDF = async () => {
  ...
  if (selectedEmployees.length === 0) {
    toast.error("Please select at least one employee", { id: "print-select-required" });
    return;
  }
  ...
  const selectedEmpRows = selectedEmployees
    .map((empId) => employees.find((e) => e._id === empId))
    .filter(Boolean);

  const uniqueDepts = [...new Set(selectedEmpRows.map((emp) => emp.departmentName || "All"))];
  const batchCache = {};
    ...
    uniqueDepts.map(async (dept) => {
      try {
        const res = await axios.get(
          `/api/payslips/check-batch?month=${tMonthName}&year=${tYear}&filterDept=${dept}`
        );
        batchCache[dept] = res.data.exists ? res.data.data : null;
      } catch {
        batchCache[dept] = null;
      }
    })
  );

  for (let i = 0; i < selectedEmpRows.length; i++) {
    const emp = selectedEmpRows[i];
    if (!emp) continue;

    // Fetch latest payslip data
    const payslip = await fetchLatestPayslip(emp, batchCache[emp.departmentName || "All"]);
    if (!payslip) continue;
...
const fetchLatestPayslip = async (emp, preloadedBatchHeader = null) => {
  try {
    const [tYear, tMonthNum] = selectedMonth.split("-");
    const tMonthName = monthNames[Number(tMonthNum) - 1];

    let batchHeader = preloadedBatchHeader;
    if (!batchHeader) {
      // Added filterDept to the URL to ensure it finds the correct batch
      const res = await axios.get(
        `/api/payslips/check-batch?month=${tMonthName}&year=${tYear}&filterDept=${emp.departmentName || "All"}`
      );
...
useEffect(() => {
  const checkMonthPayslip = async () => {
    if (!selectedMonth) {
      setHasPayslipForMonth(false);
      return;
    }
    ...
    try {
      ...
      const tMonthName = monthNames[Number(tMonthNum) - 1];
      const res = await axios.get(
        `/api/payslips/check-batch?month=${tMonthName}&year=${tYear}&filterDept=All`
      );
      setHasPayslipForMonth(!!res?.data?.exists);
    } catch (err) {
      setHasPayslipForMonth(false);
    }
  };
...
const canPrint = !!selectedMonth && hasPayslipForMonth;
  return (
    <>
